...
class _MrDetailsBodyState extends State<_MrDetailsBody>
  ...
  Widget _detailGrid(PatientModel p) {
    ...
    while (i < items.length) {
      ...
      if (item['full'] == true) {
        ...
        final next =
        (i + 1 < items.length && items[i + 1]['full'] != true)
            ? items[i + 1]
            : null;
        if (next != null) {
          rows.add(Row(children: [
      ...
      child: Column(children: [
        ...
        SizedBox(
          ...
          child: TabBarView(controller: _tabController, children: [
            _wideSearchTab(),
            SingleChildScrollView(
              child: _HistoryContent(patient: patient),
            ),
          ]),
        ),
      ]),
    ...
    return Container(
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          height: 44,
          decoration: BoxDecoration(
            color: _patient != null ? _tealLight : const Color(0xFFF8FAFC),
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
                color: _patient != null ? _teal : _teal.withOpacity(0.35),
                width: _patient != null ? 1.5 : 1),
          ),
          child: Row(children: [
            const SizedBox(width: 10),
            Icon(Icons.badge_outlined,
                color:
                _patient != null ? _teal : const Color(0xFFCBD5E0),
                size: 16),
            const SizedBox(width: 8),
            Expanded(
              child: TextField(
  ...
  //     Expanded(
  //         child: _statCard(
  //             '${p.totalVisits}', 'Total Visits', _teal, Icons.history_rounded)),
  //     const SizedBox(width: 10),
  //     Expanded(
  //         child: _statCard(
  //             '${p.visitsToday}', 'Today', _green, Icons.today_rounded)),
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  color: _tealLight, borderRadius: BorderRadius.circular(7)),
              child:
              const Icon(Icons.history_rounded, color: _teal, size: 14),
            ),
            const SizedBox(width: 8),
            const Text('Visit History',
                style: TextStyle(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            child: const Icon(Icons.badge_outlined, color: _teal, size: 36),
          ),
          const SizedBox(height: 16),
          const Text('Enter an MR Number',
              style: TextStyle(
        ...
        child: Column(children: [
          ...
          const Padding(
            ...
            child: Align(
              ...
              child: Text('Type at least 2 characters',
                  style:
                  TextStyle(fontSize: 10, color: Color(0xFFBDBDBD))),
            ),
          ),
          const SizedBox(height: 8),
          const Divider(height: 1, color: _border),
...
class _HistoryContent extends StatelessWidget {
  ...
  // ── Date formatter: any input → "14 Mar 2026" ─────────────────────────────
  String _formatDate(String? raw) {
    ...
    try {
      ...
      if (raw.contains('/')) {
        ...
        dt = DateTime.tryParse(raw); // yyyy-MM-dd or ISO
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // ── Caption with total count ─────────────────────────────────────────
        Padding(
          padding: const EdgeInsets.fromLTRB(14, 12, 14, 4),
          child: Row(children: [
            const Icon(Icons.history_rounded, size: 13, color: _teal),
        ...
        ListView.builder(
          ...
          itemBuilder: (_, i) {
            final v = visits[i];
            final isLast = i == visits.length - 1;
            final formattedDate = _formatDate(v.date);
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              // Row 1: OPD service name
                              Text(
                                v.opdService ?? 'Consultation',
                                style: const TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.w700,
                                    color: _textDark),
